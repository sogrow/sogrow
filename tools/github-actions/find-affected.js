...
const workspace = new Workspaces(join(__dirname, '../..')).readWorkspaceConfiguration()

const projects = execSync(`npx nx print-affected --select projects`)
  .toString('utf-8')
  .trim()
  .split(', ')
  .filter((project) => !!project)

const affected = projects.filter((project) => workspace.projects[project].tags?.includes(tag))
